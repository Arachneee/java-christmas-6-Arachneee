...
class DiscountEventTypeTest {
    ...
    @DisplayName("크리스마스 디데이 할인은 날짜에 따른 할인 금액을 계산할 수 있다.")
    @ParameterizedTest
    @CsvSource(value = {"1,1000", "2,1100", "3,1200", "24,3300", "25,3400"}, delimiter = ',')
    void christmasCalculateAmount(int day, int discountTarget) {
        // given
    ...
    @DisplayName("크리스마스 디데이 할인은 주문 날짜가 25일 이내이면 할인이 적용된다.")
    @ParameterizedTest
    @ValueSource(ints = {1, 2, 3, 4, 5, 6, 7, 15, 20, 23, 24, 25})
    void christmasCalculateBenefits(int day) {
        // given
        Day orderDay = Day.from(day);
    ...
    @DisplayName("크리스마스 디데이 할인은 주문 날짜가 25일을 넘어가면 할인 적용이 안된다.")
    @ParameterizedTest
    @ValueSource(ints = {26, 27, 28, 29, 30, 31})
    void christmasCalculateBenefitsOverDay(int day) {
        // given
        Day orderDay = Day.from(day);
        Order order = Order.of(orderDay, Map.of(Menu.ICE_CREAM, 10));
    ...
    @DisplayName("평일 할인은 평일에 디저트 메뉴 1개당 2023원 할인 합계를 계산할 수 있다.")
    @ParameterizedTest
    @MethodSource("weekDayMenuProvider")
    void weekdayCalculateAmount(Map<Menu, Integer> menuCount, int discountTarget) {
        // given
        Day day = Day.from(3);
    ...
    @DisplayName("평일 할인은 평일에 할인이 적용된다.")
    @ParameterizedTest
    @ValueSource(ints = {3, 4, 5, 6, 7,
            ...
            24, 25, 26, 27, 28,
            31})
    void weekdayCalculateAmountWeekend(int day) {
        // given
        Day orderDay = Day.from(day);
        Order order = Order.of(orderDay, Map.of(Menu.ICE_CREAM, 10));
    ...
    @DisplayName("평일 할인은 주말에 할인 적용이 불가능하다.")
    @ParameterizedTest
    @ValueSource(ints = {1, 2, 8, 9, 15, 16, 22, 23, 29, 30})
    void weekdayCalculateBenefitsWeekday(int day) {
        // given
        Day orderDay = Day.from(day);
    ...
    @DisplayName("주말 할인은 주말에 메인 메뉴 1개당 2023원 할인 합계를 계산할 수 있다.")
    @ParameterizedTest
    @MethodSource("weekendMenuProvider")
    void weekendCalculateBenefits(Map<Menu, Integer> menuCount, int discountTarget) {
        // given
        Day day = Day.from(1);
        Order order = Order.of(day, menuCount);
    ...
    @DisplayName("주말 할인은 평일에 할인이 적용되지 않는다.")
    @ParameterizedTest
    @ValueSource(ints = {3, 4, 5, 6, 7,
            10, 11, 12, 13, 14,
            17, 18, 19, 20, 21,
    ...
    @DisplayName("주말 할인은 주말에 할인 적용된다.")
    @ParameterizedTest
    @ValueSource(ints = {1, 2, 8, 9, 15, 16, 22, 23, 29, 30})
